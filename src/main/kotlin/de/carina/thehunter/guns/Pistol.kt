...
object Pistol : Gun {
    ...
    fun reloadGun(player: Player) {
        if (magazine.getOrDefault(player, 0) >= GamesHandler.playerInGames[player]!!.gameItems.guns["pistol-ammo"]!!)
            return
    ...
    private fun reload(player: Player) {
        Bukkit.getScheduler().scheduleSyncDelayedTask(TheHunter.instance, {
            ...
            if (amount + old >= GamesHandler.playerInGames[player]!!.gameItems.guns["pistol-ammo"]!!) {
