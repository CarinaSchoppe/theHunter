 ...
 * File created on 14.04.22, 00:24 by Carina The Latest changes made by Carina on 14.04.22, 00:24 All contents of "EggBomb.kt" are protected by copyright. The copyright law, unless expressly indicated otherwise, is
...
class EggBomb : Listener {
    ...
    fun createEggBomb(): ItemStack {
        return ItemBuilder(Material.EGG).addDisplayName(TheHunter.PREFIX + "§eEggBomb").addAmount(Random().nextInt(1, (TheHunter.instance.settings.settingsMap["egg-bomb-amount"] as Int) + 1)).addEnchantment(Enchantment.DURABILITY, 1).addLore(
            listOf(
                (TheHunter.instance.messages.messagesMap["egg-bomb-message"] as String).replace("%power%", TheHunter.instance.settings.settingsMap["egg-bomb-radius"].toString())
            )
        ).build()
    }
